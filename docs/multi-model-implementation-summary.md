...
- **AIProviderClient Implementation**
  - Supports seedream-v4, google:4@1, and qwen-image
...
- Structured logging still uses the core `logging` module instead of the shared observability helper
...
1. **Observability** (High Impact)
   - Switch to `observability.get_logger` and emit structured fields (model, duration, payload size, error)
...
4. **Logging/metrics** flow through the shared observability stack
